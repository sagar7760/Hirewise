...
// PUT update HR user
router.put('/:hrId', auth, authorize('admin'), async (req, res) => {
  try {
...
router.delete('/:hrId', auth, authorize('admin'), async (req, res) => {
  try {
    const { hrId } = req.params;
...
router.put('/:hrId/status', auth, authorize('admin'), async (req, res) => {
  try {
    const { hrId } = req.params;
